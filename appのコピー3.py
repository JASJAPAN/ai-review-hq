...
APP_VERSION = "v4"  # 反映確認用: /health で表示
...
# ---- 追加店舗の自動登録(既にあれば何もしない・管理画面での編集が常に優先) ----
EXTRA_STORES = [
    ("四季家 天文館店", "shikiya-tenmonkan",
     "https://search.google.com/local/writereview?placeid=ChIJp4N-2FBhPjURZeEYumZDD6Q",
     ["炭火串焼きが絶品", "野菜巻き串がおいしい", "季節料理が楽しめる", "完全個室でゆっくりできる",
      "天文館での飲み会にぴったり", "鹿児島観光の夜ごはんに最適", "接客が丁寧", "飲み放題が充実"]),
    ("さつま高原 天文館店", "satsuma-kogen-tenmonkan",
     "https://search.google.com/local/writereview?placeid=ChIJC3U_kmhnPjUR6GxfLH5Ji_Y",
     ["炭火焼き鳥が香ばしい", "焼き鳥食べ放題のコスパが最高", "飲み放題が充実", "天文館からのアクセスが良い",
      "宴会・飲み会にぴったり", "メニューが豊富", "接客が良い", "活気があって楽しい"]),
    ("炭炉庵 天文館店", "sumiroan-tenmonkan",
     "https://search.google.com/local/writereview?placeid=ChIJCTyzJWZhPjURANIFlgjsa5I",
     ["黒豚しゃぶしゃぶが絶品", "生本まぐろが新鮮", "完全個室でゆっくりできる", "鹿児島観光の食事に最適",
      "天文館の中心で便利", "デートや記念日にぴったり", "接客が丁寧", "飲み放題付きコースが充実"]),
    ("三島畜産 鹿児島中央駅店", "mishima-chikusan-chuoeki",
     "https://search.google.com/local/writereview?placeid=ChIJ45smWgVnPjURary0AtbppGM",
     ["鳥刺しが新鮮", "黒豚料理が絶品", "鹿児島の郷土料理が楽しめる", "個室でゆっくりできる",
      "鹿児島中央駅から近い", "出張や観光の夜ごはんに便利", "接客が丁寧", "飲み放題付きコースが充実"]),
    ("ここまで西郷 天文館店", "kokomade-saigo-tenmonkan",
     "https://search.google.com/local/writereview?placeid=ChIJdQOMX_thPjURHlkAPsH8msQ",
     ["定額制でお会計が安心", "コスパが最高", "料理の種類が豊富", "餃子がおいしい",
      "提供が早い", "鹿児島の宴会ならここ", "天文館での飲み会にぴったり", "活気があって楽しい"]),
]


def ensure_extra_stores():
    with db() as conn:
        for name, slug, url, tags in EXTRA_STORES:
            if not conn.execute("SELECT 1 FROM stores WHERE slug=?", (slug,)).fetchone():
                conn.execute(
                    "INSERT INTO stores(name,slug,google_url,tags_json,created_at) VALUES(?,?,?,?,?)",
                    (name, slug, url, json.dumps(tags, ensure_ascii=False), now()))
