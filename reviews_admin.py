...
<form method="post" action="{{url_for('reviews.bulk_approve_reports')}}" style="display:inline"
      onsubmit="return confirm('違反報告候補 {{reports|length}}件をすべて承認します（1日の送信上限に沿って順次送信）。よろしいですか？')">
  <button>違反報告を全件承認する</button></form>
...
<div class="card rep">
  ...
  <form method="post" action="{{url_for('reviews.report_approve', review_id=r.review_id)}}">
    <textarea name="report_reason" rows="4" style="width:100%">{{r.report_reason}}</textarea>
    <button style="background:#173F35;color:#fff">承認して自動報告（翌朝ワーカーが送信）</button>
  </form>
  <form method="post" action="{{url_for('reviews.report_done', review_id=r.review_id)}}" style="display:inline">
    <button>手動で報告済みにした</button></form>
...
def bulk_approve_replies():
    """承認待ちの返信をすべて確定（HPは投稿キューへ、Googleは自動投稿へ）"""
    con = db()
    con.execute("""UPDATE reviews SET reply_status = CASE WHEN platform='hotpepper' THEN 'todo' ELSE 'auto' END
                   WHERE reply_status='pending'""")
    con.commit()
    return redirect(url_for("reviews.index"))


@reviews_bp.route("/bulk/approve_reports", methods=["POST"])
def bulk_approve_reports():
    """違反報告候補をすべて承認（ワーカーが1日の上限内で順次送信）"""
    con = db()
    con.execute("UPDATE reviews SET report_status='approved' WHERE report=1 AND report_status='todo'")
    con.commit()
    return redirect(url_for("reviews.index"))


@reviews_bp.route("/<path:review_id>/report_approve", methods=["POST"])
def report_approve(review_id):
    """人がワンクリック承認 → 無人ワーカーが翌朝ホットペッパーに違反報告を送信する"""
    reason = (request.form.get("report_reason") or "").strip()[:1000]
    con = db()
    if reason:
        con.execute("UPDATE reviews SET report_status='approved', report_reason=? WHERE review_id=?", (reason, review_id))
    else:
        con.execute("UPDATE reviews SET report_status='approved' WHERE review_id=?", (review_id,))
    con.commit()
    return redirect(url_for("reviews.index"))
            ...
            try:
                ...
                con.execute("""UPDATE reviews SET reply=?, reply_status=?, report=?, policy_clause=?, report_reason=?, report_status=?
                               WHERE review_id=?""", (j["reply"], status, int(j["report"]), j.get("policy_clause", ""), reason,
                                                       (("approved" if os.environ.get("REPORT_AUTO_APPROVE") == "1" else "todo") if j["report"] else ""), r["review_id"]))
                con.commit(); n += 1
...
def tasks():
    ...
    return jsonify({
        ...
        "reports": rows("SELECT review_id,platform,store,reviewer,rating,comment,review_url,policy_clause,report_reason FROM reviews WHERE report=1 AND report_status='approved'"),
        "reports_pending_count": rows("SELECT COUNT(*) n FROM reviews WHERE report=1 AND report_status='todo'")[0]["n"],
    })
